...
import { apiClient, hostsApi, healthChecksApi, certsApi, checkInsApi, type CheckIn } from '../api/client'
...
export default function HostDetailPage() {
  ...
  // Check-in / apply history (pull model)
  ...
  const fetchCheckIns = useCallback(async () => {
    ...
    try {
      const res = await checkInsApi.list(id)
  ...
  // ── Force check-in (pull model) ───────────────────────────────────────────
  const handleForceCheckIn = async () => {
    if (!id) return
    setForceCheckInLoading(true)
    try {
      await hostsApi.forceCheckIn(id)
      showSnack('Force check-in signalled', 'success')
      // The agent runs a cycle on its SSE signal; refresh history shortly after.
      setTimeout(() => { fetchCheckIns() }, 5000)
    } catch (e: unknown) {
      const msg = (e as { response?: { data?: { error?: { message?: string } } } })
        ?.response?.data?.error?.message ?? 'Failed to signal check-in'
      showSnack(msg, 'error')
    } finally {
          ...
          {host && (<>
            ...
            <Grid size={{ xs: 12, sm: 6, md: 4 }}>
              ...
              {host?.container_runtime ? (
                <Chip size="small" label={String(host.container_runtime)} color="warning" variant="outlined" />
      ...
      <Paper sx={{ p: 3, mb: 3 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 2 }}>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            ...
            <Typography variant="h6" fontWeight={600}>Check-in &amp; Apply History</Typography>
          </Box>
          {canWrite && id && id !== 'new' && <Button
            startIcon={forceCheckInLoading ? <CircularProgress size={18} /> : <PlayArrowIcon />}
            variant="outlined"
            size="small"
        ...
        <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
          The agent pulls its assigned policy on each check-in and reports the apply result here.
        </Typography>
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell>Apply Result</TableCell>
              </TableRow>
            </TableHead>
            <TableBody>
              {checkIns.map(c => (
                <TableRow key={c.id} hover>
                  ...
                  <TableCell>{c.backend_type || '—'}</TableCell>
                  <TableCell>{c.agent_version || '—'}</TableCell>
                  <TableCell>
                    <Typography variant="body2" sx={{ fontFamily: 'monospace' }}>
                      {c.rules_hash ? c.rules_hash.slice(0, 12) : '—'}
                    </Typography>
                  </TableCell>
                  <TableCell>
                    {c.apply_success === null ? (
                      <Chip label="No apply" size="small" variant="outlined" />
                    ) : c.apply_success ? (
                      <Chip
                        label={`OK${c.applied_rule_count !== null ? ` (${c.applied_rule_count})` : ''}`}
                        color="success"
                        size="small"
                      />
                    ) : (
                      <Tooltip title={c.apply_error ?? 'Apply failed'}>
